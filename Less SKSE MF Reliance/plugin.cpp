...
void OpenSpellCraftMenu(RE::StaticFunctionTag*, RE::TESQuest* theQuest, const std::vector<RE::TESForm*> formArray,
                        RE::SpellItem* ListenerSpell) {
    ...
    if (ListenerSpell) {
        UI::SpellCraftMenu::LearnEffectsSpell = ListenerSpell;
    } else {
        logger::error("couldn't get the Learn Effects Spell");
    }
    ...
    UI::SpellCraftMenu::CraftingSpell = true;
    UI::SpellCraftMenu::SpellCraftWindow->IsOpen = true;
}

//Spell function OpenChoseSpellMenu() global native
void OpenChoseSpellMenu(RE::StaticFunctionTag*, RE::TESQuest* theQuest, RE::SpellItem* ListenerSpell) {
    if (theQuest) {
        UI::SCScriptQuest = theQuest;
    } else {
        logger::error("couldn't get the Quest");
...
//function AddEffectsInSCMenu(Form theForm) global native
void AddEffectsInSCMenu(RE::StaticFunctionTag*, RE::TESForm* theForm) {
    if (theForm) {
        auto FormType = theForm->GetFormType();
        if (FormType == RE::FormType::Spell) {
            RE::SpellItem* Spell = theForm->As<RE::SpellItem>();
            if (Spell->GetSpellType() != RE::MagicSystem::SpellType::kSpell) {
                return;
            }
            if (UI::ChangeE1) {
            ...
            } else if(UI::SpellCraftMenu::CraftingSpell) {
            ...
            auto* thePotion = theForm->As<RE::AlchemyItem>();

            if (!UI::SpellCraftMenu::CraftingSpell) {
                CallPapyrusFunction("LearnTheEffect", RE::MakeFunctionArguments((RE::AlchemyItem*)thePotion));
                return;
            } else if (UI::ChangeE1) {
                return;
            }

            for (auto& effectItem : thePotion->effects) {
                auto Duration = effectItem->GetDuration();
                auto Cost = effectItem->cost;
                auto Mag = effectItem->GetMagnitude();
                ...
                UI::SpellCraftMenu::DurationList.push_back(Duration);
                UI::SpellCraftMenu::CostList.push_back(Cost);
                UI::SpellCraftMenu::SourceSpells.push_back(nullptr);
                UI::SpellCraftMenu::DList.push_back(UI::BCostMult * mgef->data.baseCost);
            }
            UI::BCostMult = 1;
            UI::SpellCraftMenu::SpellCraftWindow->IsOpen = true;
        }
...
bool PapyrusFunctions(RE::BSScript::IVirtualMachine * vm) { 
    ...
    vm->RegisterFunction("AddEffectsInSCMenu", "EM03SKSEFunctions", AddEffectsInSCMenu);
    return true; }
...
void StopSpellCrafting(bool cancelled, bool noName) {
    ...
    CraftingSpell = false;
    auto* player = RE::PlayerCharacter::GetSingleton();
    if (!player || !LearnEffectsSpell) {
        if (!player) {
            logger::error("Player not found!");
        } else {
            logger::error("LearnEffectsSpell not received");
        }
    } else if (player->HasSpell(LearnEffectsSpell)) {
        player->RemoveSpell(LearnEffectsSpell);
    }
}
...
void __stdcall UI::SpellCraftMenu::RenderWindow() {
    ...
    if (OverideEffect) {
        ...
        const char* Lable2 = "Use the next spell you cast as a base";
        if (OverideBase) {
            ...
            if (ImGui::Button("Show Name")) {
                CallPapyrusFunction("ShowFormName", RE::MakeFunctionArguments((RE::TESForm*)Overidemgf));
            }
            Lable = "Change the Effect";
            Lable2 = "Change the effect to one from the next spell you cast";
        }
        ...
        if (ImGui::Button(Lable2)) {
            ...
            if (!player || !LearnEffectsSpell) {
                ...
                player->AddSpell(LearnEffectsSpell);
            }
            ChangeE1 = true;
            SpellCraftWindow->IsOpen = false;
            CallPapyrusFunction("CloseBook", RE::MakeFunctionArguments());
    ...
    for (std::size_t i = 0; i< SpellEffects.size(); i++) {
        ...
        ImGui::SameLine();
        ImGui::SameLine();
        std::string label2 = "Show Name##" + std::string(std::to_string(Effect->formID));
        if (ImGui::Button(label2.c_str())) {
            CallPapyrusFunction("ShowFormName", RE::MakeFunctionArguments((RE::TESForm*)Effect));
        }
        ImGui::SameLine();
        ImGui::PushID(i);
    ...
    ImGui::SameLine();
    if (ImGui::Button("Add Effects from the next spell you cast/potion you make")) {
        auto* player = RE::PlayerCharacter::GetSingleton();
        if (!player || !LearnEffectsSpell) {
            if (!player) {
                logger::error("Player not found!");
...
void GenSpellSchoolMenu(std::vector<RE::SpellItem*> School, ImVec4 Color, const char* Title, std::string_view SearchText) {  // I couldn't think of a name
    ...
    for (auto Spell : School) {
        ...
        int SpellId = Spell->formID;
        std::string label = SpellName + "##" + std::string(std::to_string(SpellId));
        ...
        ImGui::SameLine();
        std::string label2 = "Show Name##" + std::string(std::to_string(SpellId));
        if (ImGui::Button(label2.c_str())) {
            CallPapyrusFunction("ShowFormName", RE::MakeFunctionArguments((RE::TESForm*)Spell));
        }
    }
}
...
void __stdcall UI::ChoseEffectMenu::RenderWindow() {
    ...
    if (SpellToErase) {
        ...
        ImGui::SameLine();
        if (ImGui::Button("Show Name")) {
            CallPapyrusFunction("ShowFormName", RE::MakeFunctionArguments((RE::TESForm*)SpellToErase));
        }
        if (ImGui::Button("Yes")) {
            UI::ChoseEffectMenu::ChoseEffectWindow->IsOpen = false;
        ...
        Erasing ? ButtonName = "Chose a Spell by casting it"
                : ButtonName = "Add Effects from the next spell you cast/potion you make";

        if (ImGui::Button(ButtonName)) {
            auto* player = RE::PlayerCharacter::GetSingleton();
            if (!player || !SpellCraftMenu::LearnEffectsSpell) {
                if (!player) {
                    logger::error("Player not found!");
                } else {
                    logger::error("LearnEffectsSpell not received");
                }
            } else if (!player->HasSpell(SpellCraftMenu::LearnEffectsSpell)) {
                player->AddSpell(SpellCraftMenu::LearnEffectsSpell);
            }
            ChoseEffectWindow->IsOpen = false;
            CallPapyrusFunction("CloseBook", RE::MakeFunctionArguments());
        }
        ...
        if (!LearnedEffects.empty() && !ChangeE1) {
            ...
            for (auto Effect : LearnedEffects) {
                ...
                int EffectId = Effect->formID;
                std::string label = EffectName + "##" + std::string(std::to_string(EffectId));
                if (ImGui::Button(label.c_str())) {
                ...
                ImGui::SameLine();
                std::string label2 = "Show Name##" + std::string(std::to_string(EffectId));
                if (ImGui::Button(label2.c_str())) {
                    CallPapyrusFunction("ShowFormName", RE::MakeFunctionArguments((RE::TESForm*)Effect));
                }
            }
